...
public class StudentController {
  ...
  public ResponseEntity<Map<String, Object>> getStudentByFirstAndLastName(
      @PathVariable String lname, @PathVariable String fname) {
  ...
  public ResponseEntity<Object> deleteBySid(@PathVariable int sid) {
    ...
    if (studentOptional == null) {
      ...
      return ResponseEntity.badRequest()
          .body("Unable to delete - student with sid: " + sid + " not found");
    } else {
  ...
  public ResponseEntity<Object> postBySid(@RequestBody Map<String, String> body,
      @PathVariable int sid) {
    Student student = studentDao.findBySid(sid);
    if (student == null) {
      logger.error("Unable to update - student with sid: " + sid + " not found");
      return ResponseEntity.badRequest()
          .body("Unable to update - student with sid: " + sid + " not found");
    }
  ...
  private void setStudentFields(Map<String, String> body, Student student) throws Exception {
    for (String x : body.keySet()) {
      switch (x) {
        ...
        case "raceEthnicity":
          student.setRaceEth(
              raceEthDao.findByRid(parseInt("raceEthnicity", body.get("raceEthnicity"))));
          break;
        case "serviceArea":
          student.setServiceArea(
              serviceAreaDao.findBySid(parseInt("serviceArea", body.get("serviceArea"))));
          break;
        case "school":
        ...
        case "psLabel":
          ...
          break;
        case "currentTeacher":
          student.setCurrTeacher(
              teacherDao.findByTid(parseInt("currentTeacher", body.get("currentTeacher"))));
          break;
        case "secondTeacher":
          student.setSecondTeacher(
              teacherDao.findByTid(parseInt("secondTeacher", body.get("secondTeacher"))));
          break;
        case "clinic":
